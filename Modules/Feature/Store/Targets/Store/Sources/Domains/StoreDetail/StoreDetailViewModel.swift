...
final class StoreDetailViewModel: BaseViewModel {
    ...
    private func saveStore(isDelete: Bool) {
        Task {
            let saveResult = await storeService.saveStore(storeId: String(state.storeId), isDelete: isDelete)
...
extension StoreDetailViewModel {
    ...
    private func pushReviewList() {
        let config = ReviewListViewModel.Config(storeId: state.storeId, storeName: state.storeDetailData?.overview.storeName, isBossStore: false)
