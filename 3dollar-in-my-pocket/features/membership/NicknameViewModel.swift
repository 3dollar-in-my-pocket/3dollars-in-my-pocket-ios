...
class NicknameViewModel: BaseViewModel {
  ...
  init(
    ...
    self.input.tapStartButton
      ...
      .do(onNext: { _ in
        GA.shared.logEvent(event: .nickname_change_button_clicked, className: NicknameVC.self)
      })
    ...
    .subscribe { [weak self] _ in
      ...
      if let error = error as? HTTPError {
        if error == HTTPError.badRequest {
          ...
          GA.shared.logEvent(event: .nickname_already_existed, className: NicknameVC.self)
        } else {
