...
final class AppEnvironmentTests: XCTestCase {
    ...
    // MARK: TH-1336 TC9 — 프로덕션에서는 절대 노출되지 않는다

    func test_TH1336_TC9_Release빌드_운영API면_디버그기능이꺼진다() {
        XCTAssertFalse(AppEnvironment.isDebugToolAvailable(isDebugBuild: false, apiURL: prodURL))
    }

    func test_TH1336_TC9_Release빌드_API_URL이비어도_디버그기능이꺼진다() {
        XCTAssertFalse(AppEnvironment.isDebugToolAvailable(isDebugBuild: false, apiURL: ""))
    }

    func test_TH1336_TC9_Release빌드_알수없는호스트면_디버그기능이꺼진다() {
        XCTAssertFalse(AppEnvironment.isDebugToolAvailable(isDebugBuild: false, apiURL: "https://example.com"))
    }

    // MARK: TH-1336 TC10 — 개발 환경에서는 노출된다

    func test_TH1336_TC10_Debug빌드면_디버그기능이켜진다() {
    ...
    func test_TH1336_TC10_Release빌드라도_dev서버면_디버그기능이켜진다() {
        XCTAssertTrue(AppEnvironment.isDebugToolAvailable(isDebugBuild: false, apiURL: devURL))
    }

    // MARK: TH-1336 TC11 — dev 서버 판별

    func test_TH1336_TC11_dev호스트만_dev서버로판정된다() {
