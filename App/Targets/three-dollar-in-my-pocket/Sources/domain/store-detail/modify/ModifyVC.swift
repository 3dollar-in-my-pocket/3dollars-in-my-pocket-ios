...
//            .disposed(by: disposeBag)
        ...
        self.viewModel.output.categories
            .bind(to: self.modifyView.categoryCollectionView.rx.items(cellIdentifier: WriteCategoryCell.registerId, cellType: WriteCategoryCell.self)) { _, category, cell in
    ...
    private func setupMenuTableView() {
        ...
        self.modifyView.menuTableView.rx
            .setDelegate(self)
            .disposed(by: disposeBag)
        self.menuDataSource = RxTableViewSectionedReloadDataSource<MenuSection> { (_, tableView, indexPath, item) in
...
extension ModifyVC: UITableViewDelegate {
    func tableView(
        ...
        let sectionCategory = self.menuDataSource.sectionModels[section].category
