...
- 수동 TC는 코드로 만들지 않는다. 자동화 대상이 아닌 것: 애니메이션·제스처, 지도(네이버맵) 렌더링, 외부 SDK(카카오 로그인·공유, 애드몹), OS 권한 팝업, 푸시 수신. 수동 TC의 확인 방법과 화면 변경 PR 공통 체크 항목은 `docs/process/manual-test-checklist.md`.
