...
- **증거**: 스크린샷·영상을 PR 본문 "증거" 섹션에 TC 번호와 함께 붙인다 (`/3dollars:pr-body` 가 `media/` orphan 브랜치에 올려 raw URL 로 건다).
...
| 티켓 | TC | 계층 | 테스트 / 증거 | 결과 |
|---|---|---|---|---|
| TH-1337 | TC1 | 유닛 | `test_TH1337_TC1_신고에성공하면_dismissRoute가발행된다` (ReportBottomSheetViewModelTests) | ✅ |
| TH-1337 | TC5 | 유닛 | `test_TH1337_TC5_삭제된가게면_서버메시지와함께_상세를닫는Route가발행된다` | ✅ |
| TH-1337 | TC8 | 자동화 | 신고 완료 토스트 → 상세 닫힘 (report.mp4) | ✅ PASS |
| TH-1337 | TC9 | 수동 | 카카오 공유로 진입 후 신고 (실기기) | ☐ 체크리스트 |
