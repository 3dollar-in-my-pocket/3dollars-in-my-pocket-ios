...
final class RegisteredStoreEmptyView: BaseView {
    private let emptyBackground = UIImageView().then {
        $0.image = UIImage(named: "img_empty_my_registered_store_background")
    ...
    private let emptyTitleLabel = UILabel().then {
        ...
        $0.text = "registered_store_empty_title".localized
    }
    
    private let emptyDescriptionLabel = UILabel().then {
        $0.textColor = Color.gray60
        $0.font = .regular(size: 12)
        $0.text = "registered_store_empty_description".localized
    }
